...
if (isset($_POST['btn-simpan'])) {
    ...
    $query4 = "INSERT INTO transaksi (outlet_id, kode_invoice, id_pelanggan, tgl, batas_waktu, diskon, status, status_bayar, id_user) VALUES ('$id_outlet', '$kode_invoice', '$id_pelanggan', '$tgl', '$batas_waktu', '$diskon', 'baru', 'belum', '$id_user')";
    ...
    if ($insert == 1) {
        ...
        $diskon_harga = ($diskon / 100) * $total;
        $total_harga = $total - $diskon_harga;
        $kode_invoice;
        $query6 = mysqli_query($conn, "SELECT * FROM transaksi WHERE kode_invoice = '" . $kode_invoice . "'");
        $transaksi = mysqli_fetch_assoc($query6);
        $id_transaksi = $transaksi['id_transaksi'];

        $query_detail = "INSERT INTO detail_transaksi (id_transaksi, id_paket, qty, total_harga) VALUES ('$id_transaksi', '$id_paket', '$qty', '$total_harga')";
